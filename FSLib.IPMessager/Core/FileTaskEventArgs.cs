...
public class FileTaskEventArgs : EventArgs
{
    ...
    /// 远程主机信息
    /// </summary>
    public Host Host => TaskInfo?.RemoteHost;

    /// <summary>
    /// 包ID
    /// </summary>
    public ulong PackageID => TaskInfo == null ? 0ul : TaskInfo.PackageID;

    /// <summary>
    /// 文件传输的方向
    /// </summary>
    public FileTransferDirection Direction => TaskInfo == null ? throw new InvalidOperationException() : TaskInfo.Direction;

    /// <summary>
    /// 发送文件时有效，等待发送的文件列表
    /// </summary>
    public List<FileTaskItem> TaskList => TaskInfo?.TaskList;
