...
public class IPMClient : IDisposable
{
    ...
    /// 获得创建客户端时所使用的线程上下文
    /// </summary>
    public SynchronizationContext ThreadContext => _context;
    ...
    /// 文件传输管理
    /// </summary>
    public Core.FileTaskManager FileTaskManager => Commander.FileTaskManager;

    /// <summary>
    /// 文件传输管理
    /// </summary>
    public Network.TCPThread FileTaskModule => Commander.FileTaskModule;
    ...
    {
        FileTaskInfo task = new(FileTransferDirection.Send, 0ul, remoteHost);
    ...
    public bool IsInnerServiceEnabled(Services.InnerService service) => GetServiceState(service) == FSLib.IPMessager.Services.ServiceState.Running;
    ...
    public ServiceState GetServiceState(InnerService service) => GetServiceState(ServiceManager.InnerServiceTypeList[service]);
    ...
    /// 返回是否需要提交线程同步信息
    /// </summary>
    internal static bool NeedPostMessage => SynchronizationContext != null;
    ...
    /// <returns></returns>
    public static Config GetDefaultConfig(params string[] addinPath) => new()
    {
        Port = 2425,
